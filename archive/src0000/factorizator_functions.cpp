...
void print_vector_d_offset(char *name,double *vect,unsigned length,unsigned offset)
{
        ...
        if(vect && length>0)
        {
                ...
                {
                        printf("%+f ",vect[(i+offset+1)%length]);
                }
                printf("%+f\n\n",vect[(offset)%length]);
        }
}
...
void print_vector_d_difference(char *name,double *vect,double *templ, double tolerance, unsigned length)
{
	unsigned i;
	printf("%s: %d\n",name,length);
	if(vect && templ && length>0)
	{
		for(i=0;i<length-1;i++)
		{
			if(vect[i] - templ[i] > tolerance)
			{
				printf("%s", RED);
			}
			else
			if(templ[i] - vect[i] > tolerance)
			{
				printf("%s", GREEN);
			}
			printf("%+f%s ",vect[i], RESET);
		}
		printf("%+f\n\n",vect[length-1]);
	}
}
